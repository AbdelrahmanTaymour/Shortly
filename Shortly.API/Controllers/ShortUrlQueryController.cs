...
public class ShortUrlQueryController(IShortUrlQueryService queryService, IUserContext userContext) : ControllerApiBase
{
    /// <summary>
    ...
    /// <response code="400">
    ///     The request parameters are invalid (e.g., invalid organizationId format, invalid pagination
    ///     parameters).
    /// </response>
    /// <response code="401">User is not authenticated.</response>
    ...
    public async Task<IActionResult> GetByOrganizationId(Guid organizationId, [FromQuery] int pageNumber = 1,
        [FromQuery] int pageSize = 50, CancellationToken cancellationToken = default)
    {
        ValidatePage(pageNumber, pageSize);
    ...
    /// <example>
    ///     GET /api/short-urls/query/anonymous?startDate=2024-01-01T00:00:00Z&amp;endDate=2024-12-31T23:59:59Z&amp;
    ...
    ///     GET /api/short-urls/query/anonymous?startDate=2024-01-01T00:00:00Z&amp;endDate=2024-12-31T23:59:59Z&amp;
    ///     pageNumber=1&amp;pageSize=30
    ///     This endpoint returns short URLs that were created by anonymous users (not logged in) within the specified date
    ///     range.
    ...
    /// <response code="400">
    ///     The request parameters are invalid (e.g., invalid date format, start date after end date, invalid
    ...
    public async Task<IActionResult> GetAnonymousUrlsByDateRange([FromQuery] DateTime startDate,
        [FromQuery] DateTime endDate, [FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 50,
        CancellationToken cancellationToken = default)
    {
        ValidatePage(pageNumber, pageSize);
        ValidateDateRange(startDate, endDate);

        var urls = await queryService.GetAnonymousUrlsByDateRangeAsync(startDate, endDate, pageNumber, pageSize,
            cancellationToken);
        return Ok(urls);
    }
    ...
    public async Task<IActionResult> GetExpired([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 50,
        CancellationToken cancellationToken = default)
    {
        ValidatePage(pageNumber, pageSize);
    ...
    [ProducesResponseType(typeof(ExceptionResponseDto), StatusCodes.Status500InternalServerError)]
    //[RequirePermission(enPermissions.ReadUrls | enPermissions.ReadOrgLinks | enPermissions.ReadOwnLinks)]
    public async Task<IActionResult> GetPrivateLinks(Guid userId, [FromQuery] int pageNumber = 1,
        [FromQuery] int pageSize = 50, CancellationToken cancellationToken = default)
    {
        ValidatePage(pageNumber, pageSize);
    ...
    /// <example>
    ///     GET /api/short-urls/query/date-range?startDate=2024-01-01T00:00:00Z&amp;endDate=2024-12-31T23:59:59Z&amp;
    ...
    ///     GET /api/short-urls/query/date-range?startDate=2024-01-01T00:00:00Z&amp;endDate=2024-12-31T23:59:59Z&amp;
    ///     pageNumber=1&amp;pageSize=40
    ///     This endpoint returns all short URLs (both anonymous and authenticated users) created within the specified date
    ///     range.
    ...
    /// </summary>
    /// <param name="userId">
    ///     Optional. The unique identifier of the user to filter duplicates for. If not provided, searches
    ///     across all users.
    /// </param>
    /// <param name="organizationId">
    ///     Optional. The unique identifier of the organization to filter duplicates for. If not
    ///     provided, search across all organizations.
    /// </param>
    /// <param name="cancellationToken">Cancellation token to cancel the operation.</param>
    ...
    ///     If neither userId nor organizationId is provided, it searches for duplicates across the entire system (admin-level
    ///     operation).
    /// </remarks>
    ...
    [ProducesResponseType(typeof(ExceptionResponseDto), StatusCodes.Status500InternalServerError)]
    //[RequirePermission(enPermissions.ReadUrls | enPermissions.ReadOrgLinks | enPermissions.ReadOwnLinks)]
    public async Task<IActionResult> GetDuplicateUrls([FromQuery] Guid? userId = null,
        [FromQuery] Guid? organizationId = null, CancellationToken cancellationToken = default)
    {
    ...
    ///     Optional. Filter to include only URLs that are older than the specified number of days. If
    ///     not provided, returns all unused URLs regardless of age.
    /// </param>
    /// <param name="pageNumber">The page number to retrieve (starting from 1). Default is 1.</param>
    /// <param name="pageSize">The number of items per page. Default is 50, maximum is 100.</param>
    /// <param name="cancellationToken">Cancellation token to cancel the operation.</param>
    ...
    /// <response code="400">
    ///     The request parameters are invalid (e.g., negative olderThanDays value, invalid pagination
    ///     parameters).
    /// </response>
    /// <response code="401">User is not authenticated.</response>
    ...
    [ProducesResponseType(typeof(ExceptionResponseDto), StatusCodes.Status500InternalServerError)]
    //[RequirePermission(enPermissions.ReadUrls | enPermissions.ReadOrgLinks | enPermissions.ReadOwnLinks)]
    public async Task<IActionResult> GetUnusedUrls([FromQuery] int? olderThanDays = null,
        [FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 50, CancellationToken cancellationToken = default)
    {
    ...
    /// <summary>
    ///     Full-featured search endpoint for a user's short URLs.
    ///     Supports free-text search, discrete filters (status, visibility),
    ///     date-range filtering, configurable sorting, and server-side pagination.
    /// </summary>
    /// <param name="request">
    ///     All parameters bound from the query-string. UserId is required;
    ///     all filter and sort parameters are optional.
    /// </param>
    /// <param name="cancellationToken">Propagated to EF Core.</param>
    /// <returns>
    ///     A <see cref="PagedResult{T}" /> envelope:
    ///     <c>{ items, totalCount, pageNumber, pageSize, totalPages, hasPrevious, hasNext }</c>
    /// </returns>
    /// <example>
    ///     GET /api/short-urls/query/search?userId=abc&amp;search=github&amp;status=active&amp;sortBy=most-clicks&amp;
    ///     pageNumber=1&amp;pageSize=20
    /// </example>
    /// <response code="200">Returns the filtered, sorted, paginated list of short URLs.</response>
    /// <response code="400">
    ///     One or more parameters are invalid (e.g., missing UserId, invalid pagination bounds,
    ///     DateFrom after DateTo).
    /// </response>
    /// <response code="401">User is not authenticated.</response>
    /// <response code="403">User does not have permission to query the specified UserId's links.</response>
    /// <response code="500">An internal server error occurred.</response>
    [HttpGet("search", Name = "SearchShortUrls")]
    [ProducesResponseType(typeof(PagedResult<ShortUrlDto>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ExceptionResponseDto), StatusCodes.Status400BadRequest)]
    [ProducesResponseType(typeof(ExceptionResponseDto), StatusCodes.Status500InternalServerError)]
    // [RequirePermission(enPermissions.ReadOwnLinks)]
    public async Task<IActionResult> Search(
        [FromQuery] ShortUrlSearchRequest request,
        CancellationToken cancellationToken = default)
    {
        var result = await queryService.SearchByFiltersAsync(userContext.CurrentUserId, request, cancellationToken);
        return Ok(result);
    }
}