...
namespace Shortly.Domain.RepositoryContract.UserAuditLog;
...
public interface IUserAuditLogRepository
{
    ...
    Task AddAsync(Entities.UserAuditLog log);
    ...
    Task<IEnumerable<Entities.UserAuditLog>> GetByUserIdAsync(Guid userId, int limit = 50,
    ...
    Task<Entities.UserAuditLog?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<Entities.UserAuditLog>> SearchAsync(
        Guid? userId = null,
        string? action = null,
        DateTime? from = null,
        DateTime? to = null,
