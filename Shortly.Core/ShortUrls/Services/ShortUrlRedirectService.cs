...
public class ShortUrlRedirectService(
    ...
    IShortUrlRedirectQueries redirectQueries,
    ...
    ILogger<ShortUrlRedirectService> logger) : IShortUrlRedirectService
{
    ...
    {
        var shortUrlRedirectInfo = await redirectQueries.GetRedirectInfoByShortCodeAsync(shortCode, cancellationToken);
        ...
        clickTrackingQueueService.EnqueueClick(shortUrlRedirectInfo.Id, shortUrlRedirectInfo.UserId, trackingData);

        // Return the redirect result immediately, while tracking executes in the background
