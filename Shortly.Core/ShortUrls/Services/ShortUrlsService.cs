    ...
    : IShortUrlsService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (shortUrl.IsActive.HasValue) existingUrl.IsActive = shortUrl.IsActive.Value;
        if (shortUrl.TrackingEnabled.HasValue) existingUrl.TrackingEnabled = shortUrl.TrackingEnabled.Value;
        if (shortUrl.ClickLimit.HasValue) existingUrl.ClickLimit = shortUrl.ClickLimit.Value;
        if (shortUrl.IsPasswordProtected.HasValue) existingUrl.IsPasswordProtected = shortUrl.IsPasswordProtected.Value;
        if (shortUrl.IsPrivate.HasValue) existingUrl.IsPrivate = shortUrl.IsPrivate.Value;
        if (!string.IsNullOrWhiteSpace(shortUrl.Password))
            existingUrl.PasswordHash = Sha256Extensions.ComputeHash(shortUrl.Password);
        if (shortUrl.Title != null) existingUrl.Title = shortUrl.Title;
        if (shortUrl.Description != null) existingUrl.Description = shortUrl.Description;
        if (shortUrl.ExpiresAt != null) existingUrl.ExpiresAt = shortUrl.ExpiresAt;
