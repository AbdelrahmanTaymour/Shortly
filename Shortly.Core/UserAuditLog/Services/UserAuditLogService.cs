...
namespace Shortly.Core.UserAuditLog.Services;
    ...
    : IUserAuditLogService
{
    ...
    {
        var log = new Domain.Entities.UserAuditLog
        {
            Id = Guid.NewGuid(),
            UserId = userId,
            Action = action,
    ...
    /// <inheritdoc/>
    public async Task<IEnumerable<Domain.Entities.UserAuditLog>> GetRecentLogsAsync(Guid userId, int limit = 50,
    ...
    /// <inheritdoc/>
    public async Task<IEnumerable<Domain.Entities.UserAuditLog>> SearchLogsAsync(Guid? userId = null, string? action = null,
