...
namespace Shortly.Core.UserAuditLog.Contracts;
...
public interface IUserAuditLogService
{
    ...
    Task<IEnumerable<Domain.Entities.UserAuditLog>> GetRecentLogsAsync(Guid userId, int limit = 50,
    ...
    Task<Domain.Entities.UserAuditLog?> GetByIdAsync(Guid id, CancellationToken cancellationToken = default);
    ...
    Task<IEnumerable<Domain.Entities.UserAuditLog>> SearchLogsAsync(
        Guid? userId = null,
        string? action = null,
        DateTime? from = null,
        DateTime? to = null,
