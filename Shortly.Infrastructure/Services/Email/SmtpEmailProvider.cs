...
namespace Shortly.Infrastructure.Services.Email;
    ...
    public async Task<Result> SendAsync(EmailRequest request)
    {
        try
        {
            ...
            if (!_generalSettings.EnableEmailSending)
            {
                ...
                return Result.Failure("Email sending is disabled");
            }
            ...
            if (!IsEmailAllowed(request.To))
            {
                ...
                return Result.Failure($"Email sending not allowed for: {request.To}");
            }
        ...
        catch (Exception ex)
        {
            ...
            return Result.Failure("SMTP sending failed", status: ResultStatus.Error);
        }
    }

    /// <inheritdoc />
    public async Task<List<Result>> SendBulkAsync(List<EmailRequest> requests)
    {
